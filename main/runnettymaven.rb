...
class TestCase
	...
	def initialize(benchmark_path, test_case_path)
		...
		@command = "ant maintain-systems all-portal start-visualvm all-grinder all-sample stop -Dskip.build.portal=true"
	end
...
class MessageBoard < TestCase
	def run_case
		copy_files
		run_command("ant maintain-systems reload-warmup-database all-portal start-visualvm all-grinder all-sample stop -Dskip.build.portal=true", @benchmark_path)
	end
end
...
class AssetCategory < Login
end

class AssetTag < Login
end
...
def get_case(test_case, benchmark_path, test_case_path)
	...
	elsif test_case.include? "tag"
		AssetTag.new(benchmark_path, test_case_path)
	elsif test_case.include? "category"
		AssetCategory.new(benchmark_path, test_case_path)
	end
end
...
def main_test_process
	...
	test_case_root_path = "/home/repo/git/benchmark/benchmark-configs/main/nettyconfigs/#{test_case_category}"
